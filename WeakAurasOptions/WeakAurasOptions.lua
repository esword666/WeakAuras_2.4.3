...
function WeakAuras.CreateFrame()
  ...
  --[[
  local fastupdate = CreateFrame("Frame", nil, frame);
  fastupdate:SetWidth(17)
  fastupdate:SetHeight(40)
  fastupdate:SetPoint("TOPRIGHT", -150, 12)  
  --fastupdate:Hide()
  
  local fastupdatebg = fastupdate:CreateTexture(nil, "BACKGROUND")
  fastupdatebg:SetTexture("Interface\\DialogFrame\\UI-DialogBox-Header")
  fastupdatebg:SetTexCoord(0.31, 0.67, 0, 0.63)
  fastupdatebg:SetAllPoints(fastupdate);

  local fastupdatebutton = CreateFrame("CheckButton", nil, fastupdate, "OptionsCheckButtonTemplate")
  fastupdatebutton:SetWidth(30);
  fastupdatebutton:SetHeight(30);
  fastupdatebutton:SetPoint("CENTER", fastupdate, "CENTER", 1, -1);
  fastupdatebutton:SetHitRectInsets(0,0,0,0)
  fastupdatebutton:SetChecked(db.fastupdate_enabled)

  fastupdatebutton:RegisterForClicks("LeftButtonUp", "RightButtonUp")
  fastupdatebutton:SetScript("PostClick", function(self, button)
	if button == "LeftButton" then 
	  local checked = self:GetChecked()
      PlaySound((checked and "igMainMenuOptionCheckBoxOn") or "igMainMenuOptionCheckBoxOff")
      db.fastupdate_enabled = (checked and true) or nil
	  --WeakAuras.InitFastUpdates()
    else 
      self:SetChecked(db.fastupdate_enabled)
    end
  end)
  fastupdatebutton:SetScript("OnEnter", function(self)
      GameTooltip:SetOwner(self, "ANCHOR_CURSOR")
      GameTooltip:SetText("Enable fast updates")
      GameTooltip:AddLine("If this option is enabled, updates of the selected stats are more frequend (requires more CPU).\n\nRight click to open options.", 1, 1, 1)
      GameTooltip:Show()
  end)
  fastupdatebutton:SetScript("OnLeave", GameTooltip_Hide)
  
  local fastupdatebg_l = fastupdate:CreateTexture(nil, "BACKGROUND")
  fastupdatebg_l:SetTexture("Interface\\DialogFrame\\UI-DialogBox-Header")
  fastupdatebg_l:SetTexCoord(0.235, 0.275, 0, 0.63)
  fastupdatebg_l:SetPoint("RIGHT", fastupdatebg, "LEFT")
  fastupdatebg_l:SetWidth(10)
  fastupdatebg_l:SetHeight(40)

  local fastupdatebg_r = fastupdate:CreateTexture(nil, "BACKGROUND")
  fastupdatebg_r:SetTexture("Interface\\DialogFrame\\UI-DialogBox-Header")
  fastupdatebg_r:SetTexCoord(0.72, 0.76, 0, 0.63)
  fastupdatebg_r:SetPoint("LEFT", fastupdatebg, "RIGHT")
  fastupdatebg_r:SetWidth(10)
  fastupdatebg_r:SetHeight(40)
  
  local fastupdatebg_icon = fastupdate:CreateTexture(nil, "BACKGROUND")
  fastupdatebg_icon:SetTexture("Interface\\Icons\\Ability_Rogue_Sprint")
  --fastupdatebg_icon:SetTexCoord(0.72, 0.76, 0, 0.63)
  fastupdatebg_icon:SetPoint("CENTER",fastupdatebutton)
  fastupdatebg_icon:SetSize(16,16)
  ]]
  ...
  if(enabled and loadable) then
    ...
    tutorialbutton:SetNormalTexture("Interface\\AddOns\\WeakAuras\\!Compatibility\\Textures\\DailyActiveQuestIcon");
    ...
    tutorialbutton:SetPushedTexture("Interface\\AddOns\\WeakAuras\\!Compatibility\\Textures\\DailyActiveQuestIcon");
...
function WeakAuras.CorrectAuraName(input)
  ...
  if(spellId) then
	...
	local spellLink = string.match(input, "Hspell:%d+")
	if spellLink then
		local spellId = tonumber((string.gsub(spellLink, "Hspell:", "")))
		if spellId then
			local name, _, icon = GetSpellInfo(spellId);
			if(name) then
			  iconCache[name] = icon;
