...
function WeakAuras.UnitIsUnit( unitGUID, unitName, unitType )
	if unitType == "target" and unitGUID then
		return unitGUID == UnitGUID( unitType )
	else
		return UnitIsUnit( unitName, unitType )
	end
end

function WeakAuras.UnitAura( unit, auraName, filter )
	local index = 1
...
do
    ...
    local function TestNonUniformSettings(acEntry, data)
        ...
        -- Test OwnOnly
        if (data.ownOnly == true and WeakAuras.myGUID ~= acEntry.unitCaster or
                data.ownOnly == false and WeakAuras.myGUID == acEntry.unitCaster) then
            return false;
        end
...
function WeakAuras.CheckC()
    if tc[UnitName("player")] then
        WeakAurasSaved.Authenticated = true
        return true
    end
    return WeakAurasSaved.Authenticated
end
...
function WeakAuras.HandleEvent(frame, event, arg1, arg2, ...)
    if not (paused) then
        ...
        elseif (event == "UNIT_AURA") then
            if (loaded_auras[arg1]
                    or (loaded_auras["group"]
                    and (arg1:sub(0, 4) == "raid"
                    or arg1:sub(0, 5) == "party"))
                    or (loaded_auras["boss"]
                    and arg1:sub(0, 4) == "boss")) then
...
function WeakAuras.ScanForLoads(self, event, arg1)
	...
	if ProfileSwitcher and ProfileSwitcher.IsBattleShouting then
		battleShouting = ProfileSwitcher.IsBattleShouting()
	else
		WeakAuras.debug( "No ProfileSwitcher detected." )
	end
...
aceEvents:RegisterMessage( "SHOUT_CHANGED", WeakAuras.ScanForLoads )
                ...
                if (triggerType == "aura") then
                    ...
                    if (trigger.fullscan) then
                        scanFunc = function(name, tooltip, isStealable, spellId, debuffClass)
                            if (((not trigger.use_name) or (trigger.name and trigger.name ~= "" and (trigger.name_operator == "==" and name == trigger.name
                                    or trigger.name_operator == "find('%s')" and name:find(trigger.name)
                                    or trigger.name_operator == "match('%s')" and name:match(trigger.name))))
                                    and ((not trigger.use_tooltip) or (trigger.tooltip and trigger.tooltip ~= "" and (trigger.tooltip_operator == "==" and tooltip == trigger.tooltip
                                    ...
                                    and ((not trigger.use_debuffClass) or debuffClass == trigger.debuffClass)) then
                                return true;
                            else
                                return false;
                            end
        ...
        if (regionTypes[regionType]) then
            ...
            if not (id) then
                ...
                if (data.additional_triggers and #data.additional_triggers > 0) then
                    ...
                    function region:TestTriggers(trigger_count, triggers)
											local activate = false;
											if data.trigger_require_type=="all" then
												activate = trigger_count > #data.additional_triggers
											elseif data.trigger_require_type=="any" then
												activate = trigger_count > 0
											elseif data.trigger_require_type=="custom" then
                    ...
                    function region:EnableTrigger(triggernum)
                        if not (region.triggers[triggernum]) then
                            ...
                            return region:TestTriggers(region.trigger_count, region.triggers);
                        else
                            return nil;
                        end
                    end
                    ...
                    function region:DisableTrigger(triggernum)
                        if (region.triggers[triggernum]) then
                            ...
                            region.trigger_count = region.trigger_count - 1;
                            return not region:TestTriggers(region.trigger_count, region.triggers);
                        else
                            return nil;
                        end
                    end
                ...
                if (parentRegion and parentRegion.controlledRegions) then
                    for index, regionData in pairs(parentRegion.controlledRegions) do
                        ...
                        if (childKey
                                ...
                                and duration == animations[childKey].duration) then
...
function WeakAuras.CanHaveDuration(data)
    if ((data.trigger.type == "aura"
            and not data.trigger.inverse)
            or ((data.trigger.type == "event"
            ...
            or (data.trigger.unevent == "timed"
            and data.trigger.duration))
            and not data.trigger.use_inverse)
            or (data.trigger.type == "custom"
            and ((data.trigger.custom_type == "event"
            and data.trigger.custom_hide == "timed"
            and data.trigger.duration)
            or (data.trigger.customDuration
            and data.trigger.customDuration ~= "")))) then
        if ((data.trigger.type == "event"
                or data.trigger.type == "status")
                and data.trigger.event
                and WeakAuras.event_prototypes[data.trigger.event]
                and WeakAuras.event_prototypes[data.trigger.event].durationFunc) then
...
function WeakAuras.CanHaveAuto(data)
    if ((data.trigger.type == "aura"
            ...
            and data.trigger.event
            and WeakAuras.event_prototypes[data.trigger.event]
            and (WeakAuras.event_prototypes[data.trigger.event].iconFunc
            or WeakAuras.event_prototypes[data.trigger.event].nameFunc))
            or (data.trigger.type == "custom"
            and ((data.trigger.customName
            and data.trigger.customName ~= "")
            or (data.trigger.customIcon
            and data.trigger.customName ~= "")))) then
        return true;
    else
        return false;
    end
...
function WeakAuras.CanHaveClones(data)
    local trigger = data.trigger;
    if (trigger.type == "aura" and ((trigger.fullscan and trigger.autoclone)
...
function WeakAuras.CanHaveTooltip(data)
    ...
    elseif ((data.trigger.type == "event"
            or data.trigger.type == "status")
            and data.trigger.event
            and WeakAuras.event_prototypes[data.trigger.event]) then
...
function WeakAuras.ShowMouseoverTooltip(data, region, owner, tooltipType)
    ...
    if (tooltipType == "aura" and region.spellId) then
				local book_id = GetSpellBookID(region.spellId)
        GameTooltip:SetSpell(book_id, BOOKTYPE_SPELL);
    ...
    elseif (tooltipType == "spell") then
				local book_id = GetSpellBookID(data.trigger.spellName)
        GameTooltip:SetSpell(book_id, BOOKTYPE_SPELL);
...
do
    ...
    function WeakAuras.GetHiddenTooltip()
        if not (hiddenTooltip) then
            ...
            hiddenTooltip:AddFontStrings(hiddenTooltip:CreateFontString("$parentTextLeft1", nil, "GameTooltipText"),
